...
description: >
  ...
  accent, the three-step gray text ladder, Lazzer (display/numerals/chrome) + Lazzer (body),
...
colors:
  ...
  text-muted: "#E0C7FF"
  text-light: "#9A9A9A"
  accent-light: "rgba(193, 144, 255, 0.08)"
  accent-medium: "rgba(193, 144, 255, 0.15)"
  border: "rgba(193, 144, 255, 0.2)"
  card-bg: "rgba(193, 144, 255, 0.04)"
  positive: "#059669"
  negative: "#dc2626"
...
typography:
  # — reading ramp (Lazzer body + Lazzer chrome) —
  body:    { fontFamily: "Lazzer", cqw: 0.85, weight: 400, lineHeight: 1.6, color: "text-muted" }
  h4-eyebrow:{ fontFamily: "Lazzer", cqw: 0.8, weight: 600, tracking: "0.08em", upper: true, color: "primary" }
  tag:     { fontFamily: "Lazzer", px: 12, weight: 500, color: "primary" }
  counter: { fontFamily: "Lazzer", px: 13, weight: 500, tracking: "0.05em", color: "text-muted" }
  # — display / numerical ramp (Lazzer, near-black headings / cobalt numerals) —
  h3:      { fontFamily: "Lazzer", cqw: 1.25, weight: 500, lineHeight: 1.3, tracking: "-0.02em", color: "text" }
  stat-num:{ fontFamily: "Lazzer", cqw: 1.9, weight: 700, lineHeight: 1.0, color: "primary" }
  blockquote:{ fontFamily: "Lazzer", cqw: 2.4, weight: 500, lineHeight: 1.35, color: "text" }
  h2:      { fontFamily: "Lazzer", cqw: 2.6, weight: 600, lineHeight: 1.1, tracking: "-0.02em", color: "text" }
  metric-value:{ fontFamily: "Lazzer", cqw: 3.0, weight: 700, lineHeight: 1.0, color: "primary" }
  h1:      { fontFamily: "Lazzer", cqw: 4.2, weight: 700, lineHeight: 1.08, tracking: "-0.02em", color: "text" }
  quote-mark:{ fontFamily: "Lazzer", cqw: 8.0, weight: 700, lineHeight: 0.5, color: "primary", opacity: 0.15 }
...
- **Fonts** — already set to **Lazzer** (display) / **Factor** (body); ignore any preset font name lingering in prose.
- **Weights** — the brand font ships `{400, 500, 600, 700, 900}` only; every weight is clamped to these — ignore higher preset weights (e.g. 600/700) in prose.
...
The voice is two faces in fixed roles: **Lazzer** (display, every numeral, all chrome —
eyebrows uppercase 0.08em) and **Lazzer** (body, muted gray, line 1.6). Headlines are near-black;
...
- **Lazzer** (display/numerals/chrome) + **Lazzer** (body) — near-black headlines, cobalt numerals.
...
Two ramps. The **reading ramp** (Lazzer body 0.85cqw muted; Lazzer eyebrow uppercase 0.08em
cobalt) carries copy + chrome; the **display/numerical ramp** (Lazzer `h3` 1.25cqw → `h1`
...
- **Headlines near-black, −0.02em**; **eyebrows cobalt, uppercase, 0.08em**; **numerals cobalt 600–700**; **body Lazzer 400 muted, line 1.6**. No italic, no uppercase body, no cobalt headline.
...
**Composes** accent-line, meta, h1, body sub. **Focal** a 2-line Lazzer `h1` near-black, left,
...
**Focal** a row of tinted cards — cobalt `metric-value` + Lazzer label + muted desc + optional
...
blockquote, cite. **Focal** a Lazzer `blockquote` near-black under a 15%-opacity cobalt
...
Lazzer body column beside a cobalt-8% highlight block (4px cobalt left rule) carrying an inline pull
...
a Lazzer `h1` near-black, centered, with the one solid cobalt `cta-button` pill below. **Accent**
...
- Use **tinted cards** (4% fill, 20% border, 10–14px radius, no shadow); body Lazzer 400 muted, line 1.6.
...
- **Type** — Lazzer headings −0.02em near-black, cobalt eyebrows 0.08em + numerals; Lazzer body muted line 1.6; ≥1.4cqw floor.
...
- **Lazzer + Lazzer via Google Fonts.** CJK pairing (Noto Sans SC 700 display / Noto Serif SC 400 body) carries over; the eyebrow's uppercase+tracking signal weakens in CJK — pair it with the accent-line.
...
```html
<style>
@font-face{font-family:"Lazzer";font-weight:700;font-style:normal;font-display:block;src:url("assets/fonts/Lazzer-Bold.woff") format("woff");}
@font-face{font-family:"Lazzer";font-weight:800;font-style:normal;font-display:block;src:url("assets/fonts/Lazzer-ExtraBold.woff") format("woff");}
@font-face{font-family:"Lazzer";font-weight:500;font-style:normal;font-display:block;src:url("assets/fonts/Lazzer-Medium.woff") format("woff");}
@font-face{font-family:"Lazzer";font-weight:400;font-style:normal;font-display:block;src:url("assets/fonts/Lazzer-Regular.woff") format("woff");}
@font-face{font-family:"Lazzer";font-weight:600;font-style:normal;font-display:block;src:url("assets/fonts/Lazzer-SemiBold.woff") format("woff");}
</style>
```
